...
* **Implementation Logic**:
  ...
  - **Adaptive Voice Tone (BUG-V1, `tts-params.ts`)**: a second, TTS-facing layer separate from `persona.ts`'s LLM-wording coaching — collapses the 12 emotion labels into a 7-mode tone directive (`appealing`, `dominant`, `pleasing`, `empathetic`, `playful`, `serious`, `neutral`) so the agent's *voice*, not just its wording, reacts to the caller. Resolution order: (1) a task-critical keyword in the caller's utterance (`CONFIG.taskCritical`, reusing `taskCriticality()` from `importance.ts`) forces `serious` regardless of mood; (2) an active `policy.ts` escalation/acknowledgeFirst directive forces `appealing`; (3) otherwise a straight emotion→tone lookup. Drives two levers: pause/pacing text shaping (`applyEmotionProsody()`, works for every tenant on any TTS provider) and real ElevenLabs `voice_settings` (`stability`/`style`/`speed` — confirmed-documented ElevenLabs fields) for tenants with ElevenLabs configured. Deepgram's TTS REST API has no speed/pitch parameter at all (verified against the `@deepgram/sdk` `SpeakV1Request` type — only `text`/`model`/`encoding`/`sample_rate`/`container`/`bit_rate`/`callback`/`tag`), which is why the fix routes real acoustic control through ElevenLabs rather than Deepgram. "Flirty" was deliberately excluded as a tone (professionalism/liability risk for tenant businesses); "playful" covers the same intent. Threaded `policy`/caller text into the telephony path (`stream-handler.ts`), which previously never passed `policy` to TTS at all on real calls.
* **Files & Directories**:
