...
exports.getEmotionPersona = getEmotionPersona;
exports.formatPersonaBlock = formatPersonaBlock;
// ─── Per-label persona definitions ───────────────────────────────────────────
var PERSONA_MAP = {
    anger: {
        tone: "Calm, sincere, and de-escalating. The caller is angry.",
        openingStyle: "Lead with a genuine apology — not a procedural one. Offer a concrete action immediately.",
        languageRules: [
        ...
        example: "I'm truly sorry this happened. Let me fix this for you right now.",
    },
    frustration: {
        tone: "Patient and solution-focused. Acknowledge pain, then act.",
        openingStyle: "Validate the caller's experience with one sentence, then immediately offer a concrete next step.",
        languageRules: [
        ...
        example: "I can see why this is frustrating — here's exactly what I'll do right now.",
    },
    distress: {
        tone: "Gentle, calm, and safety-first. The caller is distressed.",
        openingStyle: "Acknowledge their distress softly. Slow down. Escalate to a human proactively.",
        languageRules: [
        ...
        example: "I can hear this has been really difficult. I want to help — let me connect you with someone who can give you their full attention.",
    },
    sadness: {
        tone: "Warm, empathetic, unhurried. Give them space.",
        openingStyle: "Acknowledge their sadness before anything else. Do not rush to solutions.",
        languageRules: [
        ...
        example: "I'm sorry to hear you're going through this. Please take your time — I'm here to help.",
    },
    fear: {
        tone: "Reassuring and confident. Replace uncertainty with clarity.",
        openingStyle: "State clearly what you CAN do. Avoid uncertainty words. Be the calm voice they need.",
        languageRules: [
        ...
        example: "Here is exactly what will happen: I will resolve this in the next 5 minutes.",
    },
    confusion: {
        tone: "Clear, simple, patient. One step at a time.",
        openingStyle: "Ask one focused question or explain one thing. Confirm understanding before moving on.",
        languageRules: [
        ...
        example: "Let me explain this one step at a time. First — does this [X] sound right to you?",
    },
    joy: {
        tone: "Warm and upbeat. Match their energy briefly, then assist.",
        openingStyle: "Acknowledge their positive energy naturally, then pivot to helping efficiently.",
        languageRules: [
        ...
        example: "That's wonderful to hear! I'd love to help make your day even better.",
    },
    gratitude: {
        tone: "Gracious and genuine. Accept thanks without deflecting.",
        openingStyle: "Accept the thanks genuinely, then offer continued help naturally.",
        languageRules: [
        ...
        example: "I'm really glad that helped. Is there anything else I can take care of for you today?",
    },
    excitement: {
        tone: "Energetic, celebratory, and genuinely enthusiastic. Match their energy!",
        openingStyle: "Celebrate with them sincerely. Share their enthusiasm, then offer to help with whatever comes next.",
        languageRules: [
    ...
    disappointment: {
        ...
        openingStyle: "Acknowledge the disappointment first. Don't minimize their feelings. Then offer a constructive path forward.",
        languageRules: [
        ...
        example: "I understand that's really disappointing. Let me see what I can do to help from here.",
    },
    neutral: {
 ...
 */
function getEmotionPersona(emotion) {
    var _a;
    var current = emotion.current, flags = emotion.flags;
    ...
    return (_a = PERSONA_MAP[current.label]) !== null && _a !== void 0 ? _a : PERSONA_MAP.neutral;
}
/**
 * Formats an EmotionPersona into a structured LLM coaching block.
 * Injected directly into the system prompt.
 */
function formatPersonaBlock(persona, emotion) {
    var current = emotion.current;
    var lines = __spreadArray([
        "CALLER EMOTIONAL STATE: ".concat(current.label.toUpperCase(), " (intensity: ").concat(current.intensity.toFixed(2), ", conf: ").concat(current.confidence.toFixed(2), ")"),
        "TONE YOU MUST ADOPT: ".concat(persona.tone),
        "OPENING STYLE: ".concat(persona.openingStyle),
        "LANGUAGE RULES:"
    ], persona.languageRules.map(function (r) { return "  - ".concat(r); }), true);
    if (persona.forbidden.length > 0) {
        lines.push("FORBIDDEN WORDS/PHRASES: ".concat(persona.forbidden.map(function (f) { return "\"".concat(f, "\""); }).join(", ")));
    }
    lines.push("EXAMPLE OPENING: \"".concat(persona.example, "\""));
    // Add trajectory context for the LLM
    var slopeDesc = emotion.trajectory.slope_a > 0.05
        ? "↑ arousal rising"
        : emotion.trajectory.slope_a < -0.05
            ? "↓ arousal falling"
            : "→ stable";
    lines.push("EMOTIONAL TRAJECTORY: ".concat(slopeDesc));
